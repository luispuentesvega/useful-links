    ...
    render () {
        return (
            <form onSubmit={this.onFormSubmit}>
                <h1>Useful Links</h1> 
                <Select options={this.props.languages} onChange={this.onChangeSelect}/>
                <Input type="text" placeholder="Link" value={this.state.link} onChange={this.onChangeLinkInput}/>
                <Input type="text" placeholder="Title" value={this.state.title} onChange={this.onChangeTitleInput}/>
...
const mapDispatchToProps = dispatch => {
    return {
        addLinkToLanguageRedux: (languageName, linkTitle, linkUrl) => dispatch(action.addLink(languageName, linkTitle, linkUrl))
    }
}
